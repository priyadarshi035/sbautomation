...
az login --service-principal -u $SERVICEPRINCIPALID -p $SERVICEPRINCIPALKEY --tenant $TENANTID >/dev/null
TOKEN=$(az account get-access-token --resource=https://servicebus.azure.net --query accessToken | tr -d '"')


purge_dlq_queue() {
    local DLQ_QUEUE_URL="https://${SERVICEBUS_NAMES}.servicebus.windows.net/$1/\$DeadLetterQueue/messages/head"
    local count=1000
    echo "INFORM: Cleaning the dead letters messages from the message queue..."
    while [[ ${count} -ge 0 ]]
    do
	...
	if [[ STATUS_CODE -ge 300 ]]; then
            echo "ERROR: Exit dead letters message queue cleaning with code ${STATUS_CODE}"
            return 1
        elif [[ STATUS_CODE -eq 204 ]]; then
            echo "INFORM: All Messages from Dead letters message queue $1 has been cleaned"
            return 0
        fi
        let count--
    done
    echo "ERROR: Exit with maxium number tries."
    return 1
}
...
Queues_Details () {
    ...
    then
       ...
       echo $DEAD_LETTER_QUEUE | jq -c '.[]' | while read k;
	do
	DLQ_QUEUE_NAME=$(echo $k | jq .name | tr -d '"')
